...
public class CimissUtil {  
	...
	public static List<Map<String,Object>> getCimissData(Map<String,Object> map,String wsdl,String targetNamespace,String  timeoutInMilliSeconds){
		if(!map.isEmpty()) {
			String params = "";
			...
			/* 调用接口 */
			String rstData = WebsUtil.getWsString("callAPI_to_serializedStr", params.substring(0, params.length()-1), wsdl, targetNamespace,  timeoutInMilliSeconds);
			//实况数据转换
	...
	public static List<Map<String,Object>> getListMap(String rstData){
		...
		while (it.hasNext()) {
			...
			Map<String, Object> map = parseJSON2Map(json2.toString());
			//处理数据时间+8
			//map.put("Datetime", DateUtils.getDateHourAdd(DateUtils.getDate(map.get("Datetime")+"",DateUtils.FORMAT_yyyy_MM_dd_hh_mm_ss), 8));
			cimissData.add(map); 
		}
		}
		ls.clear();
